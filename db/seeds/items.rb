...
10.times do |index|
  unless Item.exists?(id: ids[index])
    item = Item.new(
      id:             ids[index],
      name:           "テスト商品A_#{ids[index]}",
      price:          prices[index],
      category_id:    category_ids[index],
      condition_id:   condition_ids[index],
      description:    "これはテスト用のテスト商品A_#{ids[index]}です。"
    )
    ...
    begin
      ...
      puts "Created Item: テスト商品A_#{ids[index]} with image"
    rescue Errno::ENOENT => e
      puts "Error: Image file not found at #{image_path}"
      puts e.message
    rescue StandardError => e
      puts "Error attaching image to Item #{ids[index]}"
      puts e.message
    end
  end
end